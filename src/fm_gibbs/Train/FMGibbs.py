...
def gibbs_sampling(
    ...
    n_iter=100, n_pretrain=5, logger=None, record_error=False, check_error=False, rng=None
):
    if rng is None:
    ...
    for mcs in range(n_iter):
        ...
        mu_w = rng.normal(m_w_star, tau2_theta_star * sigma2_w)
        ...
        for f in range(k):
            ...
            mu_v[f] = rng.normal(m_v_f_star, tau2_theta_star * sigma2_v[f])
        ...
        b_new = rng.normal(mu2_b_star, sigma2_b_star)
        e = e + b_new - b
        b = b_new
        ...
        for i in range(d):
            ...
            w_i_new = rng.normal(mu2_w_i_star, sigma2_w_i_star)
            e = e + (w_i_new - w[i]) * h
            w[i] = w_i_new
            ...
            for f in range(k):
                ...
                v_if_new = rng.normal(mu2_v_if_star, sigma2_v_if_star)
